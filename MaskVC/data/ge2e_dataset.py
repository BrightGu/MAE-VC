...
from MaskVC.data.infinite_dataloader import InfiniteDataLoader,infinite_iterator

class GE2EDataset1(Dataset):
    ...
    def __init__(
        ...
        for figure in self.figures:
            for item in self.speaker_infos[figure]:
                self.label_mel_list.append(item)
        self.len = len(self.label_mel_list)
    ...
    def __getitem__(self, index):
        label_features = self.label_mel_list[index]

        feats =  fixed_length(label_features[1],self.seg_len)
        spk_feats =  fixed_length(label_features[1],self.seg_len)

        return feats,spk_feats
